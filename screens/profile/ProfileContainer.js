...
const ProfileContainer = (props) => {
  ...
  axios.get(`${baseURL}users/62f8cd7b1df83bbe60782743`).then(res => {
    setUser(res.data);
  }).catch((error)=>{
...
// }
  ...
  axios.get(`${baseURL}posts/user/${userID}`).then(res => {
    setPosts(res.data);
  }).catch((error)=>{
      console.log("API Error")
  })
        ...
        <View style={styles.profile}>
          <View style={{flex:1, flexDirection:'row', alignItems:'center'}}>
            <View style={{alignItems:'center'}}>
              <Text style={[styles.text, {fontSize: 18}]} >{user.follow ? user.followed.length : 0}</Text>
              <Text style={[styles.text, {fontSize: 16}]}>Followers</Text>
            </View>
            <Image style={styles.profilePic} source={require('../../assets/photos/8.png')}/>
            <View style={{alignItems:'center'}}>
              <Text style={[styles.text, {fontSize: 18}]} >{user.following ? user.following.length : 0}</Text>
          ...
          <Text style={[styles.bio]}>Bio here is my bio poopoo and peepee and balls. I like to poop and pee and workout :P I am a proper gym lad</Text>
